...
import { calculateSweatPrice, getSweatPricePerItem } from '@/lib/pricing';
...
export default function SweatPage() {
  ...
  const [stockMessage, setStockMessage] = useState('In Stock. Ready to Ship.');  const [selectedMainImage, setSelectedMainImage] = useState('');
...
• Gender: Unisex`,
  ...
  const additionalImages: string[] = ['/pant2.png', '/pant1.png', '/pant3.png', '/pant4.png'];
  ...
  useEffect(() => {
    const fetchStock = async () => {
      try {
        const response = await fetch('/api/product-stock?productId=sweat');
        if (response.ok) {
  ...
  useEffect(() => {
    if (additionalImages.length > 0 && !selectedMainImage) {
      setSelectedMainImage(additionalImages[0]);
    }
  }, []);
  ...
  return (
    <div className="container mx-auto px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">        {/* Product Image */}
          ...
          <div className="mt-4 space-y-1">
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div className="grid grid-cols-1 gap-2 text-xs">
                ...
                <div className={`flex justify-between p-2 rounded ${quantity >= 4 ? 'bg-blue-100 font-medium' : ''}`}>
                  <span>4+ items:</span>
                  <span>235 DH each</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <FrequentlyBoughtTogether currentProductId="sweat" />
...
function SizeGuideSection() {
  ...
  const sizeGuideImages = [
    { size: 'Small', image: '/SMALL SERWAL.png' },
    { size: 'Medium', image: '/MEDIUM SERWAL.png' },
    { size: 'Large', image: '/large serwal.png' },
    { size: 'XL', image: '/XLARGE SERWAL.png' }
  ];
