...
import { calculateTrackSweaterPrice, getTrackSweaterPricePerItem } from '@/lib/pricing';
...
export default function TrackSweaterPage() {
  ...
  const [selectedMainImage, setSelectedMainImage] = useState('/zipup3.png');
  const [isZoomOpen, setIsZoomOpen] = useState(false);
  const [isHovering, setIsHovering] = useState(false);
  const [mousePosition, setMousePosition] = useState({ x: 50, y: 50 });
  const productSizes = ['S', 'M', 'L', 'XL'];  // Hardcoded product data
  const productData: ProductData = {
    id: 'track-sweater',
    name: 'SCORPIONX ZIPUP HOODIE MIDNIGHT BLUE',
    description: `Premium quality cropped zip-up hoodie designed for comfort and style
...
• Gender: Unisex`,
  ...
  const additionalImages: string[] = ['/zipup3.png', '/zipup1.png', '/zipup2.png', '/zipup4.png'];
  ...
  useEffect(() => {
    const fetchStock = async () => {
      try {
        const response = await fetch('/api/product-stock?productId=track-sweater');
        if (response.ok) {
          const data = await response.json();
          setStockData(data.stockInfo || {});
          setStockMessage(data.message || 'In Stock. Ready to Ship.');
        } else {
          console.error('Failed to fetch stock data');
          setStockData({});
          setStockMessage('Stock information unavailable');
        }
      } catch (error) {
  ...
  const handleAddToCart = () => {
    ...
    // Check if selected size is available (if stock data exists)
    const sizeStock = stockData?.[selectedSize];
    if (Object.keys(stockData).length > 0 && (!sizeStock || !sizeStock.available)) {
  ...
  const increaseQuantity = () => {
    ...
    const maxQuantity = sizeStock?.quantity ?? 99;
  ...
  return (
    <div className="container mx-auto px-4 py-6 sm:px-6 lg:px-8">
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div className="grid grid-cols-1 gap-2 text-xs">
                ...
                <div className={`flex justify-between p-2 rounded ${quantity === 3 ? 'bg-blue-100 font-medium' : ''}`}>
                  <span>3 items:</span>
                  <span>680 DH</span>
                </div>                <div className={`flex justify-between p-2 rounded ${quantity >= 4 ? 'bg-blue-100 font-medium' : ''}`}>
                  <span>4+ items:</span>
                  <span>215 DH each</span>
                </div>
              </div>
            </div>
          </div>
      ...
      </div>      {/* Frequently Bought Together Bundle */}
      <FrequentlyBoughtTogether currentProductId="track-sweater" />
...
function SizeGuideSection() {
  ...
  const sizeGuideImages = [
    { size: 'Small', image: '/SMALL 9OBIYA.png' },
    { size: 'Medium', image: '/MEDIUM 9OBIYA.png' },
    { size: 'Large', image: '/LARGE 9OBIYA.png' },
    { size: 'XL', image: '/XLARGE 9OBIYA.png' }
  ];
